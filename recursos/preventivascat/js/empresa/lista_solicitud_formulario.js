...
function cargarDatos(startIndex = 0, endIndex =10,currentPage) {
    fetch('http://localhost/recursos/preventivascat/lista_solicitud_formulario.php')
...
function buscarRegistrosPorFechas(fechaInicio, fechaFin, startIndex = 0, endIndex = 10,currentPage) {
    const url = `http://localhost/recursos/preventivascat/lista_solicitud_formulario.php?fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`;
...
function mostrarRegistros(data, startIndex, endIndex, currentPage) {
    ...
    paginatedData.forEach(solicitud => {
        tablaHTML += '<tr>' +
                        ...
                        '<td>' + formatearFecha(solicitud.fecha_ingreso) + '</td>' +
                    '</tr>';
    });

    tablaHTML += '<tr>' +
                    '<td><strong style="font-size: 18px;">Total de solicitudes :</strong></td>' +
                    '<td colspan="4"><strong id="totalRecords" style="font-size: 18px;">' + totalRecords + '</strong></td>' +
